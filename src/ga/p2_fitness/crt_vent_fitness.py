...
class VentFitnessCalculator(FitnessCalculator[VentHole]):
    ...
    def __init__(
        ...
        """
        Args:
            ensem_models: tuple of Ensemble models for drag, avg_temp, and max_temp.
    ...
    def calculate(self, chromosome) -> list[float]:
        drag, avg_temp, max_temp = self._ensem_predict(chromosome)
    ...
    def _ensem_predict(
        ...
        drag, drag_std = self.ensem_models[0].predict(input_matrix, return_std=True)
        drag = drag[0]
        drag_std = drag_std[0]

        avg_temp, avg_temp_std = self.ensem_models[1].predict(
            input_matrix, return_std=True
        )
        avg_temp = avg_temp[0]
        avg_temp_std = avg_temp_std[0]

        max_temp, max_temp_std = self.ensem_models[2].predict(
            input_matrix, return_std=True
        )
        max_temp = max_temp[0]
