...
from sklearn.etrble import ExtraTreesRegressor
...
class VentFitnessCalculator(FitnessCalculator[VentHole]):
    ...
    def __init__(
        ...
        """
        Args:
            etr_models: tuple of ETR models for drag, avg_temp, and max_temp.
    ...
    def calculate(self, chromosome) -> list[float]:
        drag, avg_temp, max_temp = self._etr_predict(chromosome)
    ...
    def _etr_predict(
        ...
        drag, drag_std = self.etr_models[0].predict(input_matrix, return_std=True)
        drag = drag[0]
        drag_std = drag_std[0]

        avg_temp, avg_temp_std = self.etr_models[1].predict(
            input_matrix, return_std=True
        )
        avg_temp = avg_temp[0]
        avg_temp_std = avg_temp_std[0]

        max_temp, max_temp_std = self.etr_models[2].predict(
            input_matrix, return_std=True
        )
        max_temp = max_temp[0]
